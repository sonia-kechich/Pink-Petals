/**
 * Pink Petals — ambient sound engine.
 *
 * Every sound is *synthesised* with the Web Audio API rather than streamed from
 * an audio file. This keeps the experience:
 *   • royalty-free      — it's procedural maths, nothing to license,
 *   • lazy & weightless — voices are built on demand, zero network payload,
 *   • offline-perfect   — works identically in the PWA and the Android build,
 *   • gapless           — true seamless loops, no MP3 seam or buffering.
 *
 * The engine is a module-level singleton so playback survives React unmounts
 * and in-app navigation. A single <AmbientController/> mounted in the Layout
 * drives it from the persisted store.
 */
...
/** Display catalogue — consumed by the UI. Order defines on-screen order. */
...
/** Cleanup handle returned by every generator. */
type StopFn = () => void;
/** A generator wires its graph into `out` and returns a teardown function. */
type Generator = (ctx: AudioContext, out: GainNode) => StopFn;

/** A few seconds of looping coloured noise — the bed for most textures. */
function noiseBuffer(ctx: AudioContext, color: "white" | "pink" | "brown", seconds = 4): AudioBuffer {
  ...
  if (color === "white") {
    ...
    // Brown noise — integrated white, gently leaked to avoid DC drift.
    let last = 0;
    for (let i = 0; i < len; i++) {
      const w = Math.random() * 2 - 1;
...
/** A looping noise source feeding `dest`. */
...
/** Drive an AudioParam with a slow sine LFO (depth around center). */
...
/** Schedule a recurring event with a randomised interval. Returns a stopper. */
...
/** A short percussive noise burst (droplets, crackles, page turns…). */
...
/** A soft enveloped tone — used for birds, crickets and musical notes. */
function tone(
  ctx: AudioContext,
  dest: AudioNode,
...
// A warm, calming pentatonic scale (A-major pentatonic, a couple of octaves).
...
const GENERATORS: Record<string, Generator> = {
  ...
  rain: (ctx, out) => {
    ...
    // …sprinkled with individual droplets.
  ...
  ocean: (ctx, out) => {
    ...
    // Slow swell of the surf, plus a gentle filter sweep riding with it.
  ...
  wind: (ctx, out) => {
    ...
    const stopSweep = lfo(ctx, bp.frequency, 0.06, 380, 620); // howl rising & falling
    const stopGust = lfo(ctx, amp.gain, 0.08, 0.22, 0.34); // gusts
  ...
  forest: (ctx, out) => {
    ...
    // …with occasional distant birdsong.
  ...
  birds: (ctx, out) => {
    ...
    const bed = ctx.createGain();
    bed.gain.value = 0.05; // faint dawn air
    const src = noiseSource(ctx, "pink", hp);
  ...
  cafe: (ctx, out) => {
    ...
    // …with the occasional soft clink of cups.
  ...
  library: (ctx, out) => {
    ...
    // …broken only by the rare turn of a page.
  ...
  clock: (ctx, out) => {
    ...
    const stop = scheduler(1000, 1000, () => {
      high = !high; // gentle tick / tock alternation
  ...
  piano: (ctx, out) => {
    ...
    const stop = scheduler(1700, 3600, () => {
      ...
      note(root, 0.16);
      if (Math.random() > 0.55) window.setTimeout(() => note(pick(PENTATONIC), 0.1), 180 + Math.random() * 260); // soft harmony
    });
  ...
  instrumental: (ctx, out) => {
    // A slow, breathing pad of stacked sine partials (a gentle A-major chord).
    ...
    const oscs = freqs.map((f, i) => {
      ...
      o.type = "sine";
      o.frequency.value = f;
      o.detune.value = (i - 1.5) * 4; // slight chorus
    ...
    const stopBreath = lfo(ctx, pad.gain, 0.05, 0.5, 0.6); // slow swell
    // Occasional pentatonic melody floating over the pad.
...
const FADE = 1.4; // seconds — gentle fade in / crossfade
const STOP_FADE = 1.1; // seconds — fade out when stopping
...
class AmbientEngine {
  ...
  /** True once a context exists and audio can actually be heard. */
  get active(): boolean {
  ...
  /** Start (or crossfade to) a sound. Idempotent for the already-playing id. */
  ...
  /** Fade the current sound out and tear it down. */
  ...
  /** A soft, warm completion chime — three rising notes, never an alarm. */
  chime(): void {
    const ctx = this.ensure();
    this.unlock();
...
/** Module-level singleton — survives navigation and React remounts. */
