...
import { LogOut, User, Timer, Shield, Trash2 } from "lucide-react";
      ...
      <div className="flex flex-col gap-6">
          ...
          <Card className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>Your account</p>
                <p className="truncate text-xs" style={{ color: "var(--text-muted)" }}>{user?.email ?? "No account"}</p>
              </div>
            </div>
            ...
            <button onClick={signOut} className="btn-soft flex items-center justify-center gap-2 text-sm"><LogOut size={15} /> Sign out</button>
          </Card>
        </section>
        ...
        <section>
          ...
          <Card>
            ...
            <input className="input" placeholder="What should we call you?" value={settings.userName} onChange={(e) => updateSettings({ userName: e.target.value })} />
          </Card>
        </section>

        {/* Visible Pages */}
        <section>
          <SectionLabel>& Visible Pages</SectionLabel>
          <Card className="flex flex-col gap-3">
        ...
        {/* Timer */}
        <section>
          <SectionLabel><Timer size={14} /> Timer</SectionLabel>
          <Card className="flex flex-col gap-4">
            <Stepper label="Focus length" value={settings.pomodoroFocus} suffix="min" min={5} max={90} step={5} onChange={(v) => updateSettings({ pomodoroFocus: v })} />
            <div className="h-px" style={{ background: "var(--border)" }} />
            <Stepper label="Break length" value={settings.pomodoroBreak} suffix="min" min={1} max={30} step={1} onChange={(v) => updateSettings({ pomodoroBreak: v })} />
            <div className="h-px" style={{ background: "var(--border)" }} />
            <Toggle label="Chime on completion" value={settings.soundOnComplete} onChange={(v) => updateSettings({ soundOnComplete: v })} />
            <Toggle label="Notify when focus ends" value={settings.notifyOnComplete} onChange={(v) => updateSettings({ notifyOnComplete: v })} />
          </Card>
        </section>

        {/* Privacy */}
        <section>
          <SectionLabel><Shield size={14} /> Privacy</SectionLabel>
          <Card><p className="text-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>All data is stored locally on your device. Cloud sync is optional and encrypted.</p></Card>
        </section>
          ...
          {confirmReset ? (
            <Card className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => { resetAll(); setConfirmReset(false); }} className="btn flex-1" style={{ background: "#b08a86", color: "#fff" }}><Trash2 size={15} /> Erase everything</button>
                <button onClick={() => setConfirmReset(false)} className="btn-soft flex-1">Keep my data</button>
              </div>
            </Card>
          ) : (
            <button onClick={() => setConfirmReset(true)} className="btn-soft flex w-full items-center justify-center gap-2"><Trash2 size={15} /> Reset all data</button>
          )}
        </section>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <h2 className="muted mb-2 flex items-center gap-1.5 px-1 text-xs font-semibold uppercase tracking-wide">{children}</h2>;
}
function Label({ children }: { children: React.ReactNode }) {
  return <p className="muted mb-1.5 text-xs font-semibold uppercase tracking-wide">{children}</p>;
}
function Stepper({ label, value, suffix, min, max, step, onChange }: { label: string; value: number; suffix: string; min: number; max: number; step: number; onChange: (v: number) => void }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => onChange(Math.max(min, value - step))} aria-label={`Decrease ${label}`} className="icon-btn" style={{ background: "var(--surface-2)" }}>–</button>
        <span className="w-16 text-center text-sm font-semibold tabular-nums" style={{ color: "var(--text)" }}>{value} {suffix}</span>
        <button onClick={() => onChange(Math.min(max, value + step))} aria-label={`Increase ${label}`} className="icon-btn" style={{ background: "var(--surface-2)" }}>+</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <button onClick={() => onChange(!value)} role="switch" aria-checked={value} aria-label={label}
        ...
        <span className="absolute top-1 h-5 w-5 rounded-full transition-all duration-200"
